...
func StartMock() {
	...
	msg := fmt.Sprintf("Mock / 转发 服务运行中... 端口[%v]", config.Port)
	logger.Info(msg)
}
...
func DoHandle(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range mockItemSlice {
		// 判断条件：请求URL 和 请求方法
		if item.Path == bURL && item.Method == r.Method {
			existItem = true
	...
	// 请求的URL和请求方法对应的 Mock 项目不存在
	// 该处理用于自动记录转发的请求
	// 需要设置转发请求的目标主机
	if !existItem {
		msg = "该请求的 Mock 项目未找到，转发请求。"
		logger.Info(msg)

		// 默认 Mock 项目
		item := defaultMockItem(bURL, r.Method, config.Mock.DestHost)
		// 添加到Mock 项目切片
		mockItemSlice = append(mockItemSlice, item)

		// 转发请求
		doProxy(w, r, config.Mock.DestHost)
		// 保存响应信息
...
func defaultMockItem(path string, method string, host string) MockItem {
	...
	var item MockItem
	item.Path = path                // URL(有参数时，问号之前的部分)
...
func doMock(w http.ResponseWriter, r *http.Request, item MockItem, config *Config) {
	...
	if header == nil && !config.Mock.UseCommonHeader {
		msg := "无法获取URL对应的响应头信息。\n1. 可先访问目标主机以保存相关信息。\n2. 或手动添加响应头信息。\n3. 或使用 Mock 通用响应头。\n"
		logger.Warn(msg)
		// 返回响应
		ResponseText(w, msg)
