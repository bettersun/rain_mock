...
func UpdateMockItem(item MockItem) {
	...
	for i, v := range mockItemSlice {
		if v.Path == item.Path && v.Method == item.Method {
...
func UpdateAllMockItem(newInfoSlice []MockItem) {
	...
	for i, v := range mockItemSlice {
		for _, v2 := range newInfoSlice {
			if v.Path == v2.Path && v.Method == v2.Method {
				mockItemSlice[i] = v2
				break
			}
		}
...
func SaveMockItem(config *Config, mockItems []MockItem) {
	...
	if err != nil {
		msg := fmt.Sprintf("Mock 项目保存失败。[%s][%v]", config.Mock.MockItemFile, err)
		logger.Warn(msg)
	}

	msg := fmt.Sprintf("Mock 项目已保存。[%s]", config.Mock.MockItemFile)
	logger.Info(msg)
}
